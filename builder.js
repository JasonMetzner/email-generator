...
// Immediately executed wrapper to ensure the latest version of builder.css is loaded.
// Without this query parameter, GitHub Pages may serve a cached copy of the
// stylesheet. Appending a timestamp forces the browser to request the most
// up‑to‑date CSS. This wrapper waits for DOMContentLoaded to guarantee the
// <link> elements are present.
(function() {
  document.addEventListener('DOMContentLoaded', function() {
    const links = document.querySelectorAll('link[rel="stylesheet"]');
    links.forEach(link => {
      const href = link.getAttribute('href') || '';
      if (href.includes('builder.css')) {
        const clean = href.split('?')[0];
        link.setAttribute('href', clean + '?v=' + Date.now());
      }
    });
  });
})();
